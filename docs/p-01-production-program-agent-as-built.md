...
| PR 6 — product gate and closure record | the commit containing this document; authoritative only when its `product-agent` GateReceipt passes for the exact candidate head |
...
Stateful P-01 replacement fixtures are executed in isolated Vitest processes. This preserves each fixture's intended fresh recovery-controller/storage lifetime and avoids cross-file test-process state from becoming part of the product contract. Phase 1.1 remains the composed authority for inherited retry/successor behavior; P-01-specific tests add the bounded failure-context and replacement/recovery proofs introduced by PR 5.
