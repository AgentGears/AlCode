...
checks.push(check(
  ...
  "the exact Host verifier catalog and real verifier profile remain valid; Host sealing/nonempty verification and verification execution are already composed by the passed Phase 1.1 gate",
));
...
checks.push(check(
  ...
  "replacement fixtures run in isolated Vitest processes: dead-generation authority is retired before fresh authority, certain/quiescent recovery may continue, indeterminate mutation recovery blocks, and replacement context closes transcript gaps without inventing effect truth",
));
