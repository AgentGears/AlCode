 ...
 * and cannot create a parallel Host authority graph. Revision support is
 * optional at attach time: adaptive execution itself remains compatible with
 * Agents that negotiate only the required state/execution V2 capabilities.
 */
export class ProgramAdaptiveRevisionRuntimeV1 {
  ...
  async attachAgent(
    ...
    // Reuse the execution runtime's completed routing decision instead of
    // evaluating the adaptive predicate independently. This avoids a baseline
    // adoption race between two separate routing reads.
    const adaptive = this.runtime.agent.isCurrentConnection(sessionId, connection.generationId);
    if (!adaptive) return attached;

    const capabilities = connection.capabilities ?? [];
    if (!capabilities.includes(PROGRAM_REVISION_CAPABILITY)) {
      // Rolling-upgrade compatibility: adaptive execution requires only the
      // state/execution V2 pair. Revision planning remains unavailable until a
      // generation explicitly negotiates program_revision_v1.
      return attached;
    }
