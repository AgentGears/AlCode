...
/**
 * Request the Host-owned context decision for one provider inference.
 * The wait is request-scoped, cancellation-aware, and always unregisters its
 * listener on response, send failure, or cancellation.
 */
export async function requestInferenceContext(
  ...
  const update = await new Promise<ContextUpdate>((resolve, reject) => {
    ...
    const succeed = (response: ContextUpdate) => {
      if (settled) return;
      settled = true;
      cleanup();
      resolve(response);
