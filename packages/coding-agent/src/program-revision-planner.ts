...
import type { ProgramRevisionPlanWireV1 } from "@alcode/agent-protocol";
import type {
  ProgramRevisionProposalClientInputV1,
...
async function streamAssistant(
  ...
  for await (const event of stream) {
    if (signal?.aborted) throw new ProgramRevisionPlannerError("Revision planning was cancelled");
...
export async function runProgramRevisionPlanner(options: {
  ...
  for (let turn = 1; turn <= maxTurns; turn += 1) {
    if (options.signal?.aborted) throw new ProgramRevisionPlannerError("Revision planning was cancelled");
    ...
    for (const call of calls) {
      ...
      const result = await options.client.submitProposal({
