...
import { runProgramRevisionPlanner } from "./program-revision-planner.ts";
...
async function main(): Promise<void> {
  ...
  const processTransport = createProcessAgentTransport() as unknown as ProtocolTransport<
    AgentToHostMessageV2Aware,
    HostToAgentMessageV2Aware
  >;
  const adaptiveProtocol = createAgentProtocolBridgeV2ForTransport(processTransport);
  const revisionProtocol = createProgramRevisionProtocolClientV1(processTransport);
  ...
  revisionProtocol.onPlan((plan) => {
    ...
    }).catch((error) => reportError(error instanceof Error ? error.message : String(error), plan.sessionId));
  });
